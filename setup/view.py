...
def products_stock_alert():
    products = Product.objects.filter(alerta_estoque=True).order_by('-sells')[:10]
    lista_de_produtos_alerta_estoque = [{'nome' :product.name, 'quantidade_em_estoque' : product.quantidade_em_estoque,'vendas' : product.sells,} for product in products]
